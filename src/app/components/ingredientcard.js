    ...
    return (
        <div className="ingredient-card border p-4 rounded-lg flex flex-row flex-wrap gap-4">
            {ingredientList.map((name) => (
                <div key={name.id} className="ingredient-name w-1/5 flex items-center justify-between p-2 border rounded-lg cursor-pointer hover:bg-gray-100" onClick={console.log(1)}>
                    <span>{name.name}</span>
